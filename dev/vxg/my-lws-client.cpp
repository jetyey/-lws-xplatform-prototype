...
//=======================================================================
{
  ...
  ccinfo.port = iPort;
  ccinfo.path = "/chat";
  ccinfo.host = ccinfo.address/*lws_canonical_hostname(ipContext)*/;
    ...
    case LWS_CALLBACK_CLIENT_RECEIVE:
      {
        ...
        // Check size.
        if( (len + 1) < sizeof(l_aResponse) )
        {
          // Format response.
          memcpy(l_aResponse, in, len);
          l_aResponse[len] = '\0';
       
          // Skip 'connected' message (Web only).
          if( nullptr != strstr(l_aResponse, "connected") )  break;
    ...
    case LWS_CALLBACK_CLIENT_WRITEABLE:
      {
        ...
        // Message length.
        size_t l_Len = strlen(l_pConfig->_pMessage);
        ...
        do
        {
          ...
          if( nullptr == (buf = (unsigned char*) malloc(LWS_SEND_BUFFER_PRE_PADDING + l_Len /* LWS_SEND_BUFFER_POST_PADDING is deprecated. */)) ) break;
